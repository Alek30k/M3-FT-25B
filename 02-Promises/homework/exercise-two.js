...
function problemB () {
  ...
  // promise version
  // ???
  const promises = filenames.map(file => promisifiedReadFile(file).then(blue))
  //[Te prometo que consologueo el 1, el 2 .....8]
...
function problemC () {
  ...
  // promise version
  // ???
  filenames.reduce((p, i) => {
    return p.then((stanza) => {
      if(stanza) blue(stanza);
      return promisifiedReadFile(i)
    })
